...
int array_compare(int arr1[], int arr2[], int size)
{
    ...
    {
        if ((arr1[i] <= arr1[i + 1]) && (arr2[i] > arr2[i + 1])) return 0;
        else if ((arr1[i] > arr1[i + 1]) && (arr2[i] <= arr2[i + 1])) return 0;
    }
    return 1;
}
...
int main(void)
{
    ...
    if (array_compare(input1, input2, SIZE) == 1)
        printf("Both lists are the same relation!\n");
    else
        printf("The lists are not the same relation\n");
